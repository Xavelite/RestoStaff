(function(){
  const TeamModel = {
    ...
    employeeStatus(employee){
      ...
      if(TeamModel.approvedAbsenceCovers(employee, todayISO()))return {label:'On leave',tone:'warn'};
    ...
    visibleEmployees(employees, query, filter='all'){
      ...
      if(q)list=list.filter(employee=>`${employee.name} ${employee.firstName} ${employee.lastName} ${employeePositionName(employee)} ${employee.email} ${employee.phone} ${employee.employeeNumber}`.toLowerCase().includes(q));
