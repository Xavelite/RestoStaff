(function(){
  ...
  function metrics(){
    ...
    return `<section class="restaurant-metrics rs-page-metrics rs-weekly-metrics">
      ...
      ${Metrics.card({tone:'status',icon:issues.length?'calendar':'check',label:'Export readiness',value:`${RestaurantModel.readinessPercent()}%`,meta:issues.length?`${issues.length} setup items`:'Ready for exports'})}
    </section>`;
  }

  function venueSvg(){
    return `<svg viewBox="0 0 360 120" fill="none" aria-hidden="true">
      <path d="M22 100h316"></path>
      <path d="M88 96V50l74-23 86 23v46"></path>
      <path d="M105 59h126M117 69h32M164 69h32M211 69h22"></path>
      <path d="M120 96V76h45v20M184 96V76h44v20"></path>
      <path d="M72 96V69c0-11 8-19 19-19h11M248 96V58h44c12 0 22 10 22 22v16"></path>
      <path d="M285 96V78h18"></path>
      <path d="M53 100V76M42 76c4-12 18-12 22 0M292 50v-9M281 41h22M38 100V85M31 85c4-9 14-9 18 0M327 100V82M318 82c4-12 18-12 22 0"></path>
      <path d="M255 34h11M274 27h8M316 48h9M334 42h6"></path>
    </svg>`;
  }

  function heroChip(name,label){
    return `<span class="restaurant-hero-chip rs-entity-chip">${icon(name)}${esc(label)}</span>`;
  }
  ...
  function restaurantHead(){
    ...
    return `<header class="restaurant-main-head rs-entity-header rs-entity-header--restaurant">
      <div class="restaurant-hero-identity rs-entity-identity">
        <div class="restaurant-photo rs-entity-avatar" aria-hidden="true">${esc(restaurantName().slice(0,1))}</div>
        <div class="restaurant-hero-copy rs-entity-copy">
          <div class="restaurant-title-line rs-entity-title-line"><h2>${esc(restaurantName())}</h2>${statusIcon('active')}</div>
      ...
      <div class="restaurant-venue-illustration rs-entity-illustration">${venueSvg()}</div>
    </header>`;
  }
  ...
  function sectionHead(title,meta='',action=null){
    ...
    return `<div class="rs-section-head restaurant-section-head"><div><h3>${esc(title)}</h3>${meta?`<p>${esc(meta)}</p>`:''}</div>${button}</div>`;
  }

  function identityCard(){
    const restaurantSetup=setup();
    const general=restaurantSetup.general || {};
    return `<form class="restaurant-card restaurant-identity-card" data-restaurant-form="general">
      ${sectionHead('Identity')}
      <div class="restaurant-identity-grid">
        <label class="rs-field restaurant-inline-field"><span>Restaurant name</span><input name="name" value="${esc(data.restaurant.name || '')}"></label>
        <label class="rs-field restaurant-inline-field"><span>Manager name</span><input name="ownerName" value="${esc(data.restaurant.ownerName || '')}"></label>
        <label class="rs-field restaurant-inline-field"><span>City</span><input name="city" value="${esc(general.city || data.restaurant.city || '')}"></label>
        <label class="rs-field restaurant-inline-field"><span>Email</span><input name="email" value="${esc(general.email || '')}" type="email" placeholder="Enter email"></label>
        <label class="rs-field restaurant-inline-field wide"><span>Address</span><input name="address" value="${esc(general.address || '')}"></label>
      </div>
    </form>`;
  }
  ...
  function hoursRows(){
    ...
    return days.map(day=>{
      ...
      return `<tr class="${open?'':'is-closed'}">
        <td><label class="restaurant-day-toggle"><input type="checkbox" data-hours-day="${esc(day)}" data-hours-field="open" ${open?'checked':''}><span>${esc(day)}</span></label></td>
        <td><label class="restaurant-time-chip is-lunch">${icon('sun')}<input value="${esc(h.Lunch || '')}" data-hours-day="${esc(day)}" data-hours-field="Lunch" placeholder="-"></label></td>
        <td><label class="restaurant-time-chip is-evening">${icon('moon')}<input value="${esc(h.Evening || '')}" data-hours-day="${esc(day)}" data-hours-field="Evening" placeholder="-"></label></td>
  ...
  function openingCard(){
    return `<section class="restaurant-card restaurant-opening-card">
  ...
  function restaurantSection(){
    return `<section class="ops-tab-panel restaurant-tab-panel restaurant-profile-panel">
  ...
  function operationListHead(mode,count,action){
    ...
    return `<div class="restaurant-list-head">
      ...
      <div class="restaurant-list-actions">
        ...
        <button type="button" class="rs-action-button" data-restaurant-action="${esc(action.name)}">${icon('plus')}<span>${esc(action.label)}</span></button>
      </div>
    </div>`;
  }
  ...
  function listFooter(kind,page,total,pageSize,label){
    ...
    return `<footer class="restaurant-list-footer">
      <span>${total?`Showing ${start} to ${end} of ${total} ${label}`:`No ${label}`}</span>
      ${pageCount>1?`<div class="restaurant-pager">
        <button type="button" data-restaurant-page="${esc(kind)}" data-restaurant-page-dir="-1" ${page<=0?'disabled':''}>${icon('chevronLeft')}</button>
        <strong>${page + 1}</strong>
        <button type="button" data-restaurant-page="${esc(kind)}" data-restaurant-page-dir="1" ${page>=pageCount-1?'disabled':''}>${icon('chevronRight')}</button>
      </div>`:''}
    </footer>`;
  }
  ...
  function zoneCards(ctx){
    ...
    const cards=slice.map(zone=>{
      ...
      return `<button type="button" class="restaurant-zone-card ${active?'is-active':''} ${zone.active===false?'is-muted':''}" data-restaurant-zone="${esc(zone.id)}">
        <span class="restaurant-zone-icon">${esc(zoneIcon(zone))}</span>
        <span class="restaurant-zone-copy"><strong>${esc(zone.name)}</strong><small>${esc(zoneRoleLabel(zone))}</small></span>
        ${countLabel?`<span class="restaurant-zone-meta">${countLabel}</span>`:''}
        ${statusIcon(zone.active===false?'inactive':'active',{className:'is-inline'})}
      </button>`;
    }).join('');
    return `<section class="restaurant-list-card">
      ${operationListHead('zones',list.length,{label:'Add zone',name:'add-zone'})}
      <div class="restaurant-zone-grid">${cards || '<div class="rs-empty-state rs-empty-state--compact"><strong>No zones configured</strong><span>Add a zone.</span></div>'}</div>
  ...
  function coverageCountControl(zone,position,serviceKey,count){
    ...
    return `<div class="restaurant-coverage-count ${service==='Lunch'?'is-lunch':'is-evening'}" data-coverage-count title="${esc(service)} required count">
  ...
  function coverageEditor(zone){
    ...
    if(!positions.length){
      return `<fieldset class="restaurant-position-fieldset restaurant-coverage-fieldset"><legend>Required roles in this zone</legend><div class="rs-empty-state rs-empty-state--compact"><strong>No active positions</strong><span>Add positions before defining required roles.</span></div></fieldset>`;
    }
    ...
    return `<fieldset class="restaurant-position-fieldset restaurant-coverage-fieldset">
  ...
  function zoneShiftSettings(zone){
    ...
    return `<fieldset class="restaurant-position-fieldset restaurant-zone-shift-fieldset">
      <legend>Default times</legend>
      <div class="restaurant-zone-service-list restaurant-zone-time-list">
        <label class="restaurant-service-row is-lunch is-enabled">
          <span class="restaurant-service-toggle">${serviceLabel('Lunch')}</span>
          <input class="restaurant-service-time" name="defaultLunch" value="${esc(defaultTimes.Lunch || '')}" placeholder="Use opening hours">
        </label>
        <label class="restaurant-service-row is-evening is-enabled">
          <span class="restaurant-service-toggle">${serviceLabel('Evening')}</span>
          <input class="restaurant-service-time" name="defaultEvening" value="${esc(defaultTimes.Evening || '')}" placeholder="Use opening hours">
        </label>
      </div>
    </fieldset>`;
  }
  ...
  function zoneEditor(zone){
    if(!zone)return `<section class="restaurant-detail-card"><div class="rs-empty-state"><strong>No zones configured</strong><span>Add a zone.</span></div></section>`;
    return `<section class="restaurant-detail-card"><form data-restaurant-form="zone">
      <div class="restaurant-detail-head restaurant-detail-head--compact">
      ...
      <div class="restaurant-zone-identity-row">
        <label class="rs-field restaurant-inline-field"><span>Zone name</span><input name="name" value="${esc(zone.name)}" required></label>
  ...
  function usedInZoneChips(position){
    const list=zonesForPosition(position);
    return list.map(zone=>`<span class="restaurant-derived-chip">${esc(zone.name)}</span>`).join('') || '<div class="rs-empty-state rs-empty-state--compact"><strong>No linked zones</strong><span>Add this position in zone coverage.</span></div>';
  }
  ...
  function positionCards(ctx){
    ...
    const cards=list.map(position=>{
      ...
      return `<button type="button" class="restaurant-position-card ${active?'is-active':''} ${position.active===false?'is-muted':''}" data-restaurant-position="${esc(position.id)}">
        <span class="restaurant-position-copy"><strong>${esc(position.name)}</strong><small>${esc(positionZoneLabel(position))}</small></span>
        <span class="restaurant-position-meta">${esc(`${count} employees${cost ? ` · €${cost}/h` : ''}`)}</span>
        ${statusIcon(position.active===false?'inactive':'active',{className:'is-inline'})}
      </button>`;
    }).join('');
    return `<section class="restaurant-list-card">
      ${operationListHead('positions',list.length,{label:'Add position',name:'add-position'})}
      <div class="restaurant-position-list">${cards || '<div class="rs-empty-state rs-empty-state--compact"><strong>No positions configured</strong><span>Add a position.</span></div>'}</div>
    </section>`;
  }

  function positionEditor(position){
    return `<section class="restaurant-detail-card"><form data-restaurant-form="position">
      ...
      <div class="restaurant-detail-grid">
        <label class="rs-field restaurant-inline-field"><span>Position name</span><input name="name" value="${esc(position?.name || '')}" required></label>
        <label class="rs-field restaurant-inline-field"><span>Hourly cost</span><input name="hourlyCost" value="${esc(position?.hourlyCost || '')}" type="number" min="0" step="0.01"></label>
      ...
      <fieldset class="restaurant-position-fieldset restaurant-derived-fieldset"><legend>Used in zones</legend><div class="restaurant-chip-grid">${usedInZoneChips(position)}</div></fieldset>
    </form></section>`;
  }

  function operationsSection(ctx){
    const mode=ctx.operationMode === 'positions' ? 'positions' : 'zones';
    return `<section class="ops-tab-panel restaurant-tab-panel restaurant-operations-panel">
      <section class="restaurant-workspace-grid">
  ...
  function payrollSection(){
    ...
    const issues=RestaurantModel.issues();
    return `<section class="ops-tab-panel restaurant-tab-panel restaurant-payroll-panel"><form class="restaurant-detail-card" data-restaurant-form="payroll">
      <div class="restaurant-detail-head"><div><h3>Payroll / export rules</h3></div>${statusIcon(issues.length?'warning':'ready',{label:issues.length?`${issues.length} setup issues`:'Ready'})}</div>
      <div class="restaurant-detail-grid">
      ...
      <div class="restaurant-checklist">${issues.map(issue=>`<article>${statusIcon('warning',{label:issue,className:'is-inline'})}<strong>${esc(issue)}</strong></article>`).join('') || `<article>${statusIcon('ready',{className:'is-inline'})}<strong>All required setup items are configured.</strong></article>`}</div>
    </form></section>`;
  }
  ...
  function tabs(section){
    ...
    return items.map(([value,label])=>`<button type="button" class="ops-tab ${section===value?'is-active':''}" data-restaurant-section="${esc(value)}">${esc(label)}</button>`).join('');
  }

  function saveBar(ctx){
    const dirty = ctx.dirty ? 'is-dirty' : '';
    return `<div class="rs-save-bar rs-save-bar--inset ${dirty}" data-restaurant-save-bar>
      <span>${ctx.dirty ? 'Unsaved restaurant setup changes' : 'Restaurant setup saved'}</span>
      <button type="button" class="rs-modal-btn secondary" data-restaurant-action="cancel-restaurant" ${ctx.dirty?'':'disabled'}>Cancel</button>
      <button type="button" class="rs-modal-btn primary" data-restaurant-action="save-restaurant" ${ctx.dirty?'':'disabled'}>Save changes</button>
    </div>`;
  }

  function render(ctx){
    return `${metrics()}<section class="ops-shell-grid restaurant-layout"><main class="restaurant-main rs-panel">${restaurantHead()}<nav class="ops-tabs restaurant-tabs">${tabs(ctx.section)}</nav>${activeSection(ctx)}${saveBar(ctx)}</main></section>`;
  }
