...
function myScheduleNextShift(employee){
  if(!employee)return null;
  const now=new Date();
  let planned=[];
  days.forEach(d=>shifts.forEach(sh=>{
    if(!data.availability?.[employee.id]?.[d]?.[sh])return;
    const range=timeRangeFor(employee,d,sh);
    const [start='']=String(range||'').split('-');
    const date=dateForDay(d);
    const [y,m,day]=date.split('-').map(Number);
    const [hh='0',mm='0']=String(start).trim().split(':');
    const when=new Date(y,(m||1)-1,day||1,Number(hh)||0,Number(mm)||0,0);
    planned.push({d,sh,range,when,zone:data.assignments?.[employee.id]?.[d]?.[sh]||suggestZone(employee,sh)});
  }));
  planned.sort((a,b)=>a.when-b.when);
  return planned.find(x=>x.when>=now)||planned[0]||null;
}
function renderMySchedule(){
  const root=$('myScheduleGrid'); if(!root||!data)return;
  const e=emp(session.employeeId)||activeEmployees()[0];
  if(!e){root.innerHTML='<p class="muted">No employee selected.</p>';return;}
  if($('myScheduleName'))myScheduleName.textContent=e.name;
  if($('myScheduleAvatar'))myScheduleAvatar.textContent=String(e.name||'R').charAt(0).toUpperCase();
  if($('myScheduleWeekMeta'))myScheduleWeekMeta.textContent=weekDisplayRange();
  const published=data.status==='Published';
  const total=employeeWeekTotal(e);
  const count=days.reduce((sum,d)=>sum+shifts.filter(sh=>!!data.availability?.[e.id]?.[d]?.[sh]).length,0);
  if($('myScheduleStatus')){
    myScheduleStatus.className='my-schedule-status-card '+(published?'published':'draft');
    myScheduleStatus.innerHTML=`<strong>${published?'Published':'Draft'}</strong><span>${published?'Schedule is live.':'Mark your availability.'}</span>`;
  }
  root.innerHTML=days.map(d=>{
    const date=dateForDay(d);
    const dayLabel=d.slice(0,3);
    const cells=shifts.map(sh=>{
      const avail=!!data.availability?.[e.id]?.[d]?.[sh];
      const zone=data.assignments?.[e.id]?.[d]?.[sh]||suggestZone(e,sh)||'Unassigned';
      const range=timeRangeFor(e,d,sh);
      const h=avail?slotHours(e,d,sh):0;
      const sw=swapFor(e.id,d,sh);
      const timeText=displayTimeRange(range);
      const label=avail?(published?timeText:'Available'):'Off';
      const meta=avail?(published?`${zone} · ${fmtHours(h)}`:`${timeText} · ${zone}`):'';
      const status=sw?`<span class="my-shift-tag">${esc(sw.status)}</span>`:'';
      const style=avail?(positionStyle(e.position)+';'+zoneStyle(zone)):'';
      const check=(!published&&avail)?'<span class="my-shift-check" aria-hidden="true">✓</span>':'';
      const edit=(!published&&avail)?`<button type="button" class="my-shift-time-action" title="Set available time" onclick="myScheduleEditAvailabilityTime('${d}','${sh}',event)">Time</button>`:'';
      return `<div role="button" tabindex="0" data-day="${d}" data-shift="${sh}" class="my-shift-card ${avail?'planned':'off'} ${published?'published':'draft'} ${zoneClass(zone)}" ${styleAttr(style)} onclick="myScheduleCellClick('${d}','${sh}')" onkeydown="myScheduleCellKey(event,'${d}','${sh}')"><strong>${esc(label)}</strong>${meta?`<small>${esc(meta)}</small>`:''}${check}${edit}${status}</div>`;
    }).join('');
    return `<div class="my-schedule-row"><div class="my-day-label"><strong>${dayLabel}</strong><span>${shortDisplayDate(date)}</span></div>${cells}</div>`;
  }).join('');
  const next=myScheduleNextShift(e);
  if($('myScheduleSummary')){
    myScheduleSummary.innerHTML=`<div><span>Total week</span><strong>${fmtHours(total)}</strong></div><div><span>Shifts</span><strong>${count}</strong></div><div><span>Next shift</span><strong>${next?esc(next.d.slice(0,3)+' '+next.sh):'—'}</strong><small>${next?esc(next.range+' · '+next.zone):'No planned shift'}</small></div><div><span>Availability</span><strong>${data.submitted?.[e.id]?'Submitted':'Not submitted'}</strong></div>`;
  }
  if($('myScheduleSwaps'))myScheduleSwaps.innerHTML=swapCards('employee');
}
window.myScheduleCellClick=(d,sh)=>{
  if(!session.employeeId)return;
  slotClick(session.employeeId,d,sh,'employee');
  requestAnimationFrame(()=>{
    const btn=document.querySelector(`.my-shift-card[data-day="${d}"][data-shift="${sh}"]`);
    if(btn){btn.classList.add('just-tapped');setTimeout(()=>btn.classList.remove('just-tapped'),700)}
  });
};
window.myScheduleCellKey=(event,d,sh)=>{
  if(event.key==='Enter'||event.key===' '){event.preventDefault();myScheduleCellClick(d,sh)}
};
window.myScheduleEditAvailabilityTime=(d,sh,event)=>{
  if(event)event.stopPropagation();
  if(data.status==='Published')return;
  const id=session.employeeId;
  const e=emp(id);
  if(!id||!e)return;
  const current=timeRangeFor(e,d,sh);
  const value=prompt(`Availability time for ${d} ${sh}
Use HH:MM-HH:MM. Leave empty to use the default.`, current);
  ...
  if(!data.availability[id])data.availability[id]={};
  if(!data.availability[id][d])data.availability[id][d]={};
  ...
  data.availability[id][d][sh]=true;
  data.assignments[id][d][sh]=data.assignments[id][d][sh]||suggestZone(e,sh);
  data.assignmentTimes[id][d][sh]=range;
  data.submitted[id]=true;
  save();render();
  requestAnimationFrame(()=>{
    const btn=document.querySelector(`.my-shift-card[data-day="${d}"][data-shift="${sh}"]`);
    if(btn){btn.classList.add('just-tapped');setTimeout(()=>btn.classList.remove('just-tapped'),700)}
