-- The shared terminal records an action, not a scheduling choice. Preserve an
-- open entry's service and infer clock-ins from server-owned operational truth.
begin;
...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'clocked_in', coalesce(current_badge.clocked_in, false),
        'service_key', current_badge.service_key
      )
  ...
  left join lateral (
    select true as clocked_in, t.service_key
...
begin
  ...
  if v_open_entry.id is not null then
    ...
    if v_service_key not in ('lunch', 'evening') or not exists (
      ...
      raise exception 'No active service is available for this badge.';
    end if;
