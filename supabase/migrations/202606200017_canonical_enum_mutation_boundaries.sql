-- Phase 1 follow-up: canonical enum mutation boundaries.
--
-- Preconditions:
-- - Migration 202606200016 is applied.
-- - The canonical schedule-exception and availability RPC signatures exist.
--
-- Rollback:
-- - Restore both function definitions from the pre-deployment schema snapshot.
-- - No tables, rows or public signatures are changed by this migration.
begin;
...
do $repair_schedule_exception_enum$
declare
  v_definition text;
  v_before text;
begin
  ...
  if v_definition is null then
    raise exception 'Schedule-exception lifecycle RPC is missing.';
  end if;

  v_before := v_definition;
  v_definition := replace(
    v_definition,
    'case when p_action = ''create_by_manager'' then' || chr(13) || chr(10) ||
    '        case when coalesce((p_payload->>''approve_immediately'')::boolean, false)' ||
      chr(13) || chr(10) ||
    '          then ''approved'' else ''pending'' end' || chr(13) || chr(10) ||
    '      else ''pending'' end,',
    '(case when p_action = ''create_by_manager'' then' || chr(13) || chr(10) ||
    '        case when coalesce((p_payload->>''approve_immediately'')::boolean, false)' ||
      chr(13) || chr(10) ||
  ...
  if v_definition = v_before then
    raise exception 'Schedule-exception enum boundary drifted.';
  end if;
  execute v_definition;
end
$repair_schedule_exception_enum$;
