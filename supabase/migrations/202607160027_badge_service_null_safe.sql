-- Follow-up to 202607160025: a SELECT ... INTO with no matching row leaves the
-- service variable NULL, not ''. That made the second resolution step and the
-- time-of-day fallback both no-op (NULL = '' and NULL not in (...) are NULL),
-- so an employee with no planned shift hit a NOT NULL violation on
-- time_entries.service_key. Guard every service check with coalesce so
-- resolution — and the fallback — always run.
begin;
...
begin
  ...
  if v_open_entry.id is not null then
    ...
    -- lunch/evening service (including a NULL from an empty SELECT INTO),
    -- categorise by time of day so the clock-in always succeeds.
