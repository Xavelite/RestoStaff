...
begin
  ...
  if position('Every included Timesheet week must be approved' in v_definition) = 0
...
begin
  ...
  if v_owner_profile_id is null then
    ...
    values (v_owner_auth_user_id, 'phase6-owner-' || v_owner_auth_user_id::text || '@example.test');
    insert into public.profiles (auth_user_id, first_name, last_name, email)
    values (
      v_owner_auth_user_id,
      'Phase 6',
      'Owner',
      'phase6-owner-' || v_owner_auth_user_id::text || '@example.test'
    )
    returning id into v_owner_profile_id;
    insert into public.restaurants (workspace_slug, name, owner_profile_id)
    values (
      'phase6-anchor-' || replace(gen_random_uuid()::text, '-', ''),
      'Phase 6 authentication anchor',
      v_owner_profile_id
    )
