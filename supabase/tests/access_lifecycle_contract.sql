...
begin
  ...
  if v_restaurant_id is null then
    ...
    values (v_owner_auth_user_id, 'phase3-owner-' || v_owner_auth_user_id::text || '@example.test');
    ...
    v_active_auth_user_id := gen_random_uuid();
    insert into auth.users (id, email)
    values (v_active_auth_user_id, 'phase3-employee-' || v_active_auth_user_id::text || '@example.test');
    insert into public.profiles (auth_user_id, first_name, last_name, email)
    values (
      v_active_auth_user_id,
      'Phase 3',
      'Employee',
      'phase3-employee-' || v_active_auth_user_id::text || '@example.test'
    )
    returning id into v_active_profile_id;
    insert into public.employees (restaurant_id, display_name, active)
    values (v_restaurant_id, 'Phase 3 active employee', true)
    returning id into v_active_employee_id;
    insert into public.restaurant_memberships (restaurant_id, profile_id, role, status)
    values (v_restaurant_id, v_active_profile_id, 'employee', 'active');
