...
begin
  ...
  if v_owner_profile_id is null or v_second_profile_id is null then
    raise exception 'Phase 8 requires two authenticated profile fixtures.';
  end if;
...
begin
  ...
  exception
    when others then
      if sqlerrm not like 'Every included Actuals week must be approved%' then
        raise;
      end if;
  end;
...
begin
  ...
  exception
    when others then
      if sqlerrm not like 'Resolve missing badges before approving Actuals%' then
        raise;
      end if;
  end;
end
