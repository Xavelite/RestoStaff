...
begin
  if to_regprocedure(
    'public.save_manager_planning(uuid,date,text,jsonb,jsonb,bigint,text)'
  ) is null then
...
begin
  ...
  if v_owner_profile_id is null then
    raise exception 'Phase 4 requires an authenticated owner fixture';
  end if;
  ...
  exception
    when others then
      if sqlerrm not like 'Reopen this Actuals week%' then raise; end if;
  end;
