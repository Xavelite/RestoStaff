<script lang="ts">
  import WorkspaceCellBadge, { type WorkspaceCellBadgeIcon } from './WorkspaceCellBadge.svelte';
  ...
  const BADGE_ICON = {
    ...
    attention: 'clock',
    problem: 'warning'
  } as const satisfies Record<'neutral' | 'info' | 'ok' | 'attention' | 'problem', WorkspaceCellBadgeIcon>;
</script>

<!-- Backward-compatible semantic adapter for the shared workspace cell badge. -->
<WorkspaceCellBadge {label} {params} tone={BADGE_TONE[tone]} icon={icon ?? BADGE_ICON[tone]} />
