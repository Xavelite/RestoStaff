<script lang="ts">
  ...
  import { WORKSPACE_TEAM_CONTEXT, type WorkspaceTeamContext } from './workspace-context';
  ...
  let {
    ...
    children: Snippet<[WorkspaceTeamContext]>;
  } = $props();
  ...
  const context = $derived<WorkspaceTeamContext>({
    ...
    canManageOperations: workspace.canManageOperations,
    canViewFinancials: workspace.canViewFinancials,
    saving,
  ...
  setContext(WORKSPACE_TEAM_CONTEXT, () => context);
</script>
