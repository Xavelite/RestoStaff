...
function writeCache<T>(key: string, value: T): void {
  ...
  try {
    ...
    // Weather is optional when browser storage is unavailable.
  }
}
...
async function resolveLocation(input: WeatherLocationInput): Promise<ResolvedWeatherLocation> {
  ...
  const url = new URL(env.PUBLIC_WEATHER_GEOCODING_ENDPOINT || DEFAULT_GEOCODING_ENDPOINT);
  ...
  const best = [...(payload.results ?? [])]
    .sort((left, right) => scoreLocation(right, input) - scoreLocation(left, input))[0];
...
async function fetchForecast(location: ResolvedWeatherLocation): Promise<RestaurantWeatherSnapshot> {
  ...
  const url = new URL(env.PUBLIC_WEATHER_FORECAST_ENDPOINT || DEFAULT_FORECAST_ENDPOINT);
...
class RestaurantWeatherStore {
  ...
  async load(location: WeatherLocationInput, force = false): Promise<void> {
    ...
    if (!force && key === this.loadedKey && (this.snapshot || this.request)) {
