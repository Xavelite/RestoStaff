      ...
      <div class="cl-tablewrap">
        <table class="cl-table cl-mobile-rows">
          <thead><tr>
            ...
            {#if shown('payment')}<th class="has-menu"><WorkspaceColMenu label={t('Payment')} sortable sortDir={view.sortDir('payment')} onsort={(dir) => view.setSort('payment', dir)} filterKind="values" filterValues={paymentValues} selected={view.excluded('payment')} ontoggle={(value) => view.toggleValue('payment', value)} onselectall={(on) => view.selectAll('payment', on, paymentValues)} /></th>{/if}
            {#if shown('approval')}<th class="has-menu"><WorkspaceColMenu label={t('Needs approval')} sortable sortDir={view.sortDir('approval')} onsort={(dir) => view.setSort('approval', dir)} filterKind="values" filterValues={approvalValues} selected={view.excluded('approval')} ontoggle={(value) => view.toggleValue('approval', value)} onselectall={(on) => view.selectAll('approval', on, approvalValues)} /></th>{/if}
          ...
          {:else}
            {#each groups as group (group.key)}
              <tbody>
                {#if view.grouping}<WorkspaceGroupRow colspan={colCount} label={group.label} meta={t('{count} types', { count: group.rows.length })} collapsed={view.isCollapsed(group.key)} ontoggle={() => view.toggleGroup(group.key)} />{/if}
                {#if !view.isCollapsed(group.key)}
                  {#each group.rows as type (type.id)}
                    <tr>
                      ...
                      {#if shown('status')}<td><WorkspaceStatus label={type.active ? 'Active' : 'Archived'} tone={type.active ? 'ok' : 'attention'} /></td>{/if}
                      <td></td>
                    </tr>
                  {/each}
                {/if}
