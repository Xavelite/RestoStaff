<script lang="ts">
  ...
  // Everything under [data-design='classic'] gets the calm palette, including
  // dialogs and toasts that portal to <body>. Cleared on the way out so the
  // modern design is never left wearing it.
  //
  // Arriving here is the same statement as choosing it from the account menu,
  // so a bookmarked or shared classic link becomes this device's preference.
  // Done once on entry, not per navigation: an effect would keep re-asserting
  // "classic" while the switch is navigating away.
  onMount(() => {
    document.documentElement.dataset.design = 'classic';
    design.remember('classic');
    return () => delete document.documentElement.dataset.design;
  });

  // Role boundaries are enforced by route, not by hidden links. The classic
  // design has no employee screens yet, so an employee lands on the modern one.
  $effect(() => {
    ...
    if (module && !module.roles.includes(role)) {
      goto('/classic/home', { replaceState: true });
    }
  });

  $effect(() => {
    if (kiosk.locked && page.url.pathname !== '/classic/badge-terminal') {
      goto('/classic/badge-terminal', { replaceState: true });
    }
  });
  ...
  async function resendVerification() {
    ...
    try {
      const { error } = await supabase.auth.resend({
        type: 'signup',
        email,
        options: { emailRedirectTo: `${location.origin}/classic/home` }
      });
...
{#if auth.session}
  ...
  {:else}
    <div class="cl-app" class:is-rail={sidebarCollapsed}>
      <a class="cl-brand" href="/classic/home" aria-label="Restogogo">
