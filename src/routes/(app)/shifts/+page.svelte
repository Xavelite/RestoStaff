<script lang="ts">
  ...
  const hasPendingEdits = $derived(
    dirty || selectedTimeOffSlots.length > 0 || selectedChangeSlots.length > 0
  );
  ...
  const pendingRequestSlots = $derived(
    slots.filter(
      (slot) =>
        ...
        changeSelectedKeySet.has(slot.key)
    )
  );
  ...
  const heroSubtitle = $derived(
    availabilityMode === 'weekly_availability'
      ? 'Tap a service to mark yourself available. Use ⋯ for time off or details.'
      : availabilityMode === 'fixed_schedule'
        ? 'Your recurring schedule stays put — tap ⋯ to request a change or time off.'
        : 'Tap ⋯ on any service to request time off.'
  );
  $effect(() => {
    if (weekStart) return;
    const requested = page.url.searchParams.get('week');
    weekStart = requested && /^\d{4}-\d{2}-\d{2}$/.test(requested) ? mondayFor(requested) : mondayFor(today);
  });
  ...
  function requestTimeOffFor(slot: EmployeeWeekSlot) {
    ...
    selectedChangeSlots = selectedChangeSlots.filter((item) => item.key !== slot.key);
    selectedTimeOffSlots = toggleEmployeeSlotSelection(selectedTimeOffSlots, selection);
    feedback = '';
  }

  function requestChangeFor(slot: EmployeeWeekSlot) {
    if (blockReasonFor(slot, 'availability')) return;
    const selection = { key: slot.key, date: slot.date, serviceKey: slot.serviceKey };
  ...
  // One tap, no page-level mode to get wrong: a pending request on this slot
  // cancels it, an in-progress time-off/change basket keeps extending itself,
  // otherwise a weekly-availability slot toggles instantly and everything
  // else opens the Drawer, which is also where every action lives explicitly.
        ...
        try {
          await saveWorkPatternException({
            ...
            payload: {
              ...
              reason: actionComment.trim() || 'Availability change requested by employee'
            }
          });
          ok += 1;
        } catch (err) {
  ...
  function slotTitle(slot: EmployeeWeekSlot) {
    ...
    if (changeSelectedKeySet.has(slot.key)) return 'Change selected';
  ...
  function slotMeta(slot: EmployeeWeekSlot) {
    ...
    return slot.editable ? 'Tap to mark available' : 'Tap ⋯ for options';
  }

  function slotVisual(slot: EmployeeWeekSlot) {
    if (timeOffSelectedKeySet.has(slot.key)) return 'selected-leave';
    if (changeSelectedKeySet.has(slot.key)) return 'selected-change';
    if (slot.state === 'available') return 'available';
...
{#if snapshot && employee}
  <section class="page-shell employee-page service-page">
    <header class="page-hero service-hero">
      <div class="page-hero__copy">
        <span class="page-kicker">My service</span>
        <h1>{heroTitle}</h1>
        <p>{heroSubtitle}</p>
        <div class="page-nav">
      ...
      <aside class="glass-card week-glance" aria-label="Week glance">
        ...
        {:else}
          <p class="week-glance__next"><b>Availability</b> {submission}</p>
        {/if}
      </aside>
    </header>
  ...
  <EmployeeSlotDrawer
    ...
    isChangeSelected={selectedSlot ? changeSelectedKeySet.has(selectedSlot.key) : false}
    {saving}
    onclose={() => (slotDetailsOpen = false)}
    onToggleAvailability={() => selectedSlot && toggleAvailabilityFor(selectedSlot)}
    onRequestTimeOff={() => selectedSlot && requestTimeOffFor(selectedSlot)}
    onRequestChange={() => selectedSlot && requestChangeFor(selectedSlot)}
  ...
  <EmployeeRequestDetailsDialog
    ...
    mode={detailsMode}
    description="Optional details. The normal flow works with the restaurant default."
