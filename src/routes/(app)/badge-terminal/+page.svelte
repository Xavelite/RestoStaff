<script lang="ts">
  ...
  async function loadRoster() {
    ...
    feedback = '';
    try {
      const employees = await listBadgeRoster(workspace.activeId);
      roster = employees;
  ...
  async function verify() {
    ...
    result = null;
    try {
      verification = await verifyBadgePin(workspace.activeId, selected.employeeId, pin);
      pin = '';
    } catch (error) {
      resetChallenge();
      feedback = friendlyError(error, 'badge');
  ...
  async function completeBadge() {
    ...
    try {
      const photoUrl = proof
        ...
        : undefined;
      const recorded = await recordBadge({
        restaurantId,
        employeeId,
        token: verification.token,
        photoUrl,
...
<section class="terminal" aria-labelledby="badge-title">
  <header class="terminal-head">
    <div>
      <p>{workspace.active?.restaurant_name}</p>
